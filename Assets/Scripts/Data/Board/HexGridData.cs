...
public class CellData {
    ...
    public CellData Clone() {
        CellData newCell = new CellData(coordinates.x, coordinates.y);
...
public class HexGrid {
    ...
    public CellData GetCell(int x, int y) {
        ...
        if (cells.TryGetValue(coords, out CellData cell)) {
            return cell;
        }
        return null;
    }
    ...
    public List<Vector2Int> GetHexNeighbors(int x, int y) {
        ...
        foreach (int[] offset in offsets) {
            ...
            CellData neighborCell = GetCell(nx, ny);
    ...
    public Dictionary<Vector2Int, CellData> GetAllCells() {
        return cells;
    }
...
[CreateAssetMenu(fileName = "HexGridData", menuName = "Game/Hex Grid Data", order = 1)]
public class HexGridData : ScriptableObject {
    ...
    private class CellData {
        ...
        public CellData(global::CellData cell) {
    ...
    private void LoadWorkingGrid() {
        ...
        if (serializedCells != null && serializedCells.Length > 0) {
            foreach (var cellData in serializedCells) {
                global::CellData cell = workingGrid.GetCell(cellData.x, cellData.y);
                if (cell != null) {
...
public class HexGridDataEditor : Editor {
    ...
    private void DrawHexGrid(Rect viewRect) {
        ...
        foreach (var cellEntry in cellCenterPositions) {
            ...
            // Отримуємо клітинку
            CellData cell = workingGrid.GetCell(cellCoord.x, cellCoord.y);
